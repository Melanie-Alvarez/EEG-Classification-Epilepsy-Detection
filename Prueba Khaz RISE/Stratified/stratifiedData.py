...
archivos_ictal = [os.path.join(directorio1, f) for f in os.listdir(directorio1) if f.endswith('.mat')]
archivos_interictal = [os.path.join(directorio2, f) for f in os.listdir(directorio2) if f.endswith('.mat')]
archivos_preictal = [os.path.join(directorio3, f) for f in os.listdir(directorio3) if f.endswith('.mat')]

# Verificar si hay al menos 150 archivos .mat en total
total_archivos = len(archivos_ictal) + len(archivos_interictal) + len(archivos_preictal)
if total_archivos < 150:
    raise ValueError("No hay suficientes archivos .mat. Se necesitan al menos 150 archivos.")

# Dividir aleatoriamente el 80% para entrenamiento y 20% para prueba en cada directorio
def dividir_datos_estratificados(archivos):
    random.shuffle(archivos)
    n_train = int(0.8 * len(archivos))
    return archivos[:n_train], archivos[n_train:]

# División estratificada para cada clase
archivos_train_ictal, archivos_test_ictal = dividir_datos_estratificados(archivos_ictal)
archivos_train_interictal, archivos_test_interictal = dividir_datos_estratificados(archivos_interictal)
archivos_train_preictal, archivos_test_preictal = dividir_datos_estratificados(archivos_preictal)

# Combinar los archivos de entrenamiento y prueba
archivos_train = archivos_train_ictal + archivos_train_interictal + archivos_train_preictal
archivos_test = archivos_test_ictal + archivos_test_interictal + archivos_test_preictal
...
guardar_datos_txt(archivos_train, 'ECGKhas_Train_Strata.txt', claves_datos)
guardar_datos_txt(archivos_test, 'ECGKhas_Test_Strata.txt', claves_datos)
